...
Public Sub exportPnLs(Optional filterCriteria As Variant = "By Department", Optional matchCol As String = "P", Optional filenameCol As String = "Q", Optional columnRefCol As String = "R", Optional subFolderCol As String = "", Optional alternativeSheetNameCol As String = "")
    ...
    ' Parameters:
    '   filterCriteria - Filter to apply (e.g., "By Department", "By Group")
    '   matchCol - Column containing matching/filtering values (default "P")
    '   filenameCol - Column containing filename values (default "Q")
    '   columnRefCol - Column containing column references (default "R")
    '   subFolderCol - Column containing subfolder names (optional, empty = use main folder)
    '   alternativeSheetNameCol - Column for alternative sheet names (optional, empty = use Column D)
    '-----------------------------
    ...
    ' Safely clear any AutoFilter state
    On Error Resume Next
    If globalMasterSheet.AutoFilterMode Then
        If globalMasterSheet.FilterMode Then
            globalMasterSheet.ShowAllData
        End If
    End If
    On Error GoTo 0
    ...
    ' Get unique values from Setup sheet filename column
    setupLastRow = globalsetupSheet.Cells(globalsetupSheet.Rows.Count, filenameCol).End(xlUp).Row
    Set uniqueFilenames = CreateObject("Scripting.Dictionary")
    Set uniqueDepts = CreateObject("Scripting.Dictionary")
    Debug.Print "Reading Setup sheet from " & filenameCol & "9 to " & filenameCol & setupLastRow

    ' Collect unique filenames and departments (Column D)
    For i = 9 To setupLastRow
        filename = Trim(CStr(globalsetupSheet.Cells(i, filenameCol).value))
        dept = Trim(CStr(globalsetupSheet.Cells(i, "D").value))

        ' Only process rows where match column contains the filter criteria
        Dim columnPValue As String
        columnPValue = Trim(CStr(globalsetupSheet.Cells(i, matchCol).value))
        If InStr(1, columnPValue, CStr(filterCriteria), vbTextCompare) = 0 Then
            GoTo NextRow ' Skip this row if match column doesn't contain the filter criteria
        End If

        ' Skip blank rows in filename column
...
NextRow:
    ...
    If uniqueFilenames.Count = 0 Then
        MsgBox "No valid filenames found in Setup sheet column " & filenameCol & ". Please check your data.", vbExclamation
    ...
    For Each filenameKey In uniqueFilenames.keys
        ...
        For i = 9 To setupLastRow
            ...
            setupFilename = Trim(CStr(globalsetupSheet.Cells(i, filenameCol).value))
            setupDept = Trim(CStr(globalsetupSheet.Cells(i, "D").value))

            ' Only process rows where match column contains the filter criteria
            Dim setupColumnPValue As String
            setupColumnPValue = Trim(CStr(globalsetupSheet.Cells(i, matchCol).value))
            If InStr(1, setupColumnPValue, CStr(filterCriteria), vbTextCompare) = 0 Then
                GoTo NextSetupRow ' Skip this row if match column doesn't contain the filter criteria
            End If
...
NextSetupRow:
        ...
        For Each deptKey In filenameDepts.keys
            ...
            Set newSheet = CopySheetStructureFast(globalMasterSheet, newWorkbook, dept, columnRefCol, alternativeSheetNameCol)
            ...
            Call UpdatePnLForExportedSheets(newSheet, dept, masterDict, columnRefCol, filenameCol)

            ' Apply FilterSheets macro to this department sheet
            newSheet.Activate
            Call ApplyFilterSheetsToActiveSheet(newSheet)
        Next deptKey
        ...
        ' Create list of all departments in the new workbook BEFORE copying source sheets
        Debug.Print "=== DEPARTMENT LIST FOR WORKBOOK: " & CleanFilename & " ==="
        Dim departmentList As Object
        Set departmentList = CreateObject("Scripting.Dictionary")
        Dim wsNew As Worksheet
        Dim deptListCount As Long: deptListCount = 0
        For Each wsNew In newWorkbook.Worksheets

            ' Check if sheet has "Individual P&L" in cell A1
            On Error Resume Next
            Dim cellA1Individual As String
            cellA1Individual = Trim(CStr(wsNew.Cells(1, "A").value))
            On Error GoTo 0
            If cellA1Individual = "Individual P&L" Then

                ' Get department from cell I1
                On Error Resume Next
                Dim deptI1 As String
                deptI1 = Trim(CStr(wsNew.Cells(1, "I").value))
                On Error GoTo 0
                If deptI1 <> "" And Not departmentList.Exists(deptI1) Then
                    departmentList.Add deptI1, Nothing
                    deptListCount = deptListCount + 1
                    Debug.Print "Department " & deptListCount & ": '" & deptI1 & "' (Sheet: " & wsNew.Name & ")"
                End If
            End If
        Next wsNew
        Debug.Print "Total departments found in workbook " & CleanFilename & ": " & departmentList.Count
        Debug.Print "=== END DEPARTMENT LIST ==="

        ' Copy the 3 source sheets as last sheets (with department filtering)
        Debug.Print "Copying source sheets (Budget-FY, Actual-FY, Setup) to workbook: " & CleanFilename
        Call CopySourceSheets(newWorkbook, departmentList)

        ' Copy all VBA modules to the new workbook
        Debug.Print "Copying VBA modules to workbook: " & CleanFilename
        Call CopyAllVBAModules(newWorkbook)

        ' Clear tab colors for Individual P&L sheets
        Debug.Print "Clearing tab colors in workbook: " & CleanFilename
        Call clearTabColor(newWorkbook)

        ' Unhide buttons in the new workbook
        Debug.Print "Unhiding buttons in workbook: " & CleanFilename
        Call UnhideButtons(newWorkbook)

        ' Assign macros to buttons in the new workbook
        Debug.Print "Assigning macros to buttons in workbook: " & CleanFilename
        Call PutMacroInButton(newWorkbook)

        ' Lock summary sheets
        Debug.Print "Locking summary sheets in workbook: " & CleanFilename
        Call LockSummarySheets(newWorkbook)

        ' Lock Individual P&L sheets
        Debug.Print "Locking Individual P&L sheets in workbook: " & CleanFilename
        Call LockIndividualPLSheets(newWorkbook)
        Debug.Print "Completed creating " & filenameDepts.Count & " sheets for workbook: " & CleanFilename

        'hide other sheets
        Call hideSpecificSheetsOther(newWorkbook)

        'Collapse rows/columns in Setup sheet
        Call collapseSetupSheet(newWorkbook)

        ' Save workbook (with subfolder support)
        Dim fullPath As String, finalSavePath As String
        finalSavePath = savePath

        ' Check for subfolder if subFolderCol parameter is provided
        If subFolderCol <> "" Then

            ' Find subfolder name for this filename
            Dim subFolderName As String: subFolderName = ""
            For i = 9 To setupLastRow
                Dim checkFilename As String
                checkFilename = Trim(CStr(globalsetupSheet.Cells(i, filenameCol).value))
                If checkFilename = CStr(filenameKey) Then
                    subFolderName = Trim(CStr(globalsetupSheet.Cells(i, subFolderCol).value))
                    Exit For
                End If
            Next i

            ' Create subfolder if specified and doesn't exist
            If subFolderName <> "" And subFolderName <> "0" Then
                finalSavePath = savePath & "\" & subFolderName
                If Dir(finalSavePath, vbDirectory) = "" Then
                    MkDir finalSavePath
                    Debug.Print "Created subfolder: " & finalSavePath
                End If
            End If
        End If
        fullPath = finalSavePath & "\" & CleanFilename & ".xlsm"
...
End Function
Private Function CopySheetStructureFast(sourceSheet As Worksheet, targetWorkbook As Workbook, newSheetName As String, columnRefCol As String, alternativeSheetNameCol As String) As Worksheet
    ...
    ' Determine the actual sheet name to use
    Dim actualSheetName As String
    actualSheetName = newSheetName ' Default to department name from Column D
    
    ' Check if we should use alternative sheet name
    If alternativeSheetNameCol <> "" Then
        ' Find the department row in globalSetupSheet to get alternative name
        Dim altSetupRow As Long: altSetupRow = 0
        Dim j As Long
        For j = 9 To 1000
            If globalsetupSheet.Cells(j, "D").value = newSheetName Then
                altSetupRow = j
                Exit For
            End If
        Next j
        
        If altSetupRow > 0 Then
            Dim alternativeName As String
            alternativeName = Trim(CStr(globalsetupSheet.Cells(altSetupRow, alternativeSheetNameCol).value))
            If alternativeName <> "" And alternativeName <> "0" Then
                actualSheetName = alternativeName
                Debug.Print "  Using alternative sheet name '" & actualSheetName & "' instead of '" & newSheetName & "'"
            Else
                Debug.Print "  Alternative sheet name column is blank, using default: " & newSheetName
            End If
        End If
    End If
    ...
    Dim cleanName As String
    cleanName = CleanSheetName(actualSheetName, targetWorkbook)
    ...
    If setupRow > 0 Then
        ...
        newSheet.Cells(1, "A").value = globalsetupSheet.Cells(setupRow, "B").value ' A1 = Column B
        newSheet.Cells(2, "A").value = globalsetupSheet.Cells(setupRow, columnRefCol).value ' A2 = Column reference (column letter)

        ' A3 = Convert column letter to column number
        Dim columnLetter As String
        columnLetter = CStr(globalsetupSheet.Cells(setupRow, columnRefCol).value)
        If columnLetter <> "" And columnLetter <> "0" Then
            newSheet.Cells(3, "A").value = Range(columnLetter & "1").Column ' A3 = Column number
        Else
            newSheet.Cells(3, "A").value = 0 ' Default if no column letter
        End If
        Debug.Print "  Added setup sheet values from row " & setupRow & " to G1-G4 and A1-A3"
...
Private Sub BulkRemoveControlsAllSheets(targetWorkbook As Workbook)
    ...
    For Each ws In targetWorkbook.Worksheets
        ...
        ' Collect shapes to delete (preserve FilterButton and UnfilterButton)
        For Each shp In ws.Shapes

            ' Skip FilterButton and UnfilterButton - preserve these
            If shp.Name = "FilterButton" Or shp.Name = "UnfilterButton" Then
                Debug.Print "  Preserving button: " & shp.Name & " in sheet: " & ws.Name
            Else
                ' Delete all other shape types
...
Private Function BuildMasterDictionary(masterDict As Object, uniqueDepts As Object, columnRefCol As String, _
    ...
    For i = 9 To setupLastRow
        ...
        setupCol = Trim(CStr(globalsetupSheet.Cells(i, columnRefCol).value))  ' Column reference parameter
...
Private Sub UpdatePnLForExportedSheets(targetSheet As Worksheet, department As String, masterDict As Object, columnRefCol As String, filenameCol As String)
    ...
    For setupRowIndex = 9 To setupLastRow
        ...
        setupCol = Trim(CStr(globalsetupSheet.Cells(setupRowIndex, columnRefCol).value))  ' Column reference parameter
...
Sub updateManual()
    ...
    If lastRowBudget >= 12 Then
        ...
        ' Read all existing data in one bulk operation (A to Y columns - includes summaryType and department)
        arrBudgetData = globalBudgetFYSheet.Range("A12:Y" & lastRowBudget).value

        ' Build dictionary from existing data
        For i = 1 To UBound(arrBudgetData, 1)
            If arrBudgetData(i, 1) <> "" Then ' If lookup key exists
                Dim budgetRow(1 To 27) As Variant ' Row + 25 columns (A to Y) + AK marker
                budgetRow(1) = i + 11 ' Actual row number in sheet
                For j = 1 To 25 ' A to Y (25 columns)
                    budgetRow(j + 1) = arrBudgetData(i, j)
                Next j
                budgetRow(27) = "False" ' Initialize AK marker as False
...
Private Sub UpdateBudgetRowsWithAllocation(globalBudgetDict As Object, company_code As String, cost_center As String, customer_segment As String, venue As String, transactionCategory As String, newAmounts As Variant, ByRef processedCount As Long, ByRef maxRowBudget As Long, ledger_account As String, spend_category As String, revenue_category As String, usGaapDict As Object, manualSummaryType As String, Optional activeSheet As Worksheet = Nothing)
    ...
    ' Get department from activeSheet if provided, otherwise use original filter criteria
    Dim departmentFilter As String
    If Not activeSheet Is Nothing Then
        departmentFilter = Trim(CStr(activeSheet.Range("I1").value))
    Else
        departmentFilter = ""
    End If
    ...
    For Each budgetKey In globalBudgetDict.keys
        ...
        ' If activeSheet is provided, match based on department (Column Y)
        If Not activeSheet Is Nothing Then
            ' Match based on department (Column Y = position 26 in budgetRowData)
            If departmentFilter <> "" And Trim(CStr(budgetRowData(26))) <> departmentFilter Then isMatch = False
        Else
            ' Original filter criteria (for backward compatibility with updateManual)
    ...
    For monthIndex = 1 To 12
        ...
        If targetAmount <> 0 Then
            ...
            For Each matchKey In matchingRows.keys
                ...
                updatedRowData(27) = "True" ' Mark as updated in AK column
...
Private Sub CreateNewBudgetRow(globalBudgetDict As Object, company_code As String, cost_center As String, customer_segment As String, venue As String, transactionCategory As String, newAmounts As Variant, ByRef maxRowBudget As Long, ledger_account As String, spend_category As String, revenue_category As String, usGaapDict As Object, manualSummaryType As String)
    ...
    Dim newBudgetRow(1 To 27) As Variant ' Row + 25 columns (A to Y) + AK marker
    ...
    ' Set other columns to empty/default values for positions 23-25 (columns V-Y)
    newBudgetRow(23) = "" ' Column V
    newBudgetRow(24) = "" ' Column W
    newBudgetRow(25) = manualSummaryType ' Column X - summaryType
    newBudgetRow(26) = "" ' Column Y - Department (to be set separately if needed)

    ' Mark as updated (column AK = position 27)
    newBudgetRow(27) = "True" ' Mark new row as updated
...
Private Sub WriteBulkBudgetDataManual(globalBudgetDict As Object, maxRowBudget As Long)
    ...
    Dim bulkArray() As Variant
    ReDim bulkArray(1 To maxRowBudget - 11, 1 To 25) ' Rows 12 to maxRowBudget, Columns A to Y
    ...
    ' Write entire array to sheet in one operation
    If maxRowBudget >= 12 Then
        globalBudgetFYSheet.Range("A12:Y" & maxRowBudget).value = bulkArray

        ' Mark only truly updated rows in column AK with "True"
        For Each key In globalBudgetDict.keys
            rowData = globalBudgetDict(key)
            Dim rowNumber As Long
            rowNumber = rowData(1) ' Row number in sheet

            ' Check if row was marked as updated (position 27 for column AK)
            If UBound(rowData) >= 27 And rowData(27) = "True" Then
                globalBudgetFYSheet.Cells(rowNumber, "AK").value = "True"
            End If
        Next key
        Debug.Print "Bulk write completed for " & globalBudgetDict.Count & " budget rows and marked updated rows in column AK"
    End If

End Sub

Private Sub CopyRowsToOpexTab(maxRowBudget As Long)

    ' Copy only updated rows (where column AK = "True") from Budget-FY to FOR OPEX TAB sheet
    ' If lookup value already exists, update the existing row instead of creating a new one
    On Error Resume Next

    ' Find or create the FOR OPEX TAB sheet
    Dim opexSheet As Worksheet
    Set opexSheet = Nothing
    Set opexSheet = ThisWorkbook.Sheets("FOR OPEX TAB")
    If opexSheet Is Nothing Then
        Debug.Print "WARNING: FOR OPEX TAB sheet not found"
        Exit Sub
    End If

    ' Find the last row in FOR OPEX TAB sheet using column A
    Dim lastRowOpex As Long
    lastRowOpex = opexSheet.Cells(opexSheet.Rows.Count, "A").End(xlUp).Row

    ' If no data exists, start from row 1
    If opexSheet.Cells(1, "A").value = "" Then
        lastRowOpex = 0
    End If

    ' Build a dictionary of existing lookup values in FOR OPEX TAB sheet for fast lookup
    Dim existingLookupDict As Object
    Set existingLookupDict = CreateObject("Scripting.Dictionary")
    
    If lastRowOpex > 0 Then
        Dim i As Long
        For i = 1 To lastRowOpex
            Dim existingLookupValue As String
            existingLookupValue = Trim(CStr(opexSheet.Cells(i, "A").value))
            If existingLookupValue <> "" Then
                existingLookupDict(existingLookupValue) = i ' Store row number for this lookup value
            End If
        Next i
        Debug.Print "Found " & existingLookupDict.Count & " existing lookup values in FOR OPEX TAB sheet"
    End If

    ' Process only rows where column AK = "True"
    If maxRowBudget >= 12 Then
        Dim copiedCount As Long, updatedCount As Long
        copiedCount = 0
        updatedCount = 0

        ' Loop through Budget-FY rows and process only those marked as updated
        Dim sourceRow As Long
        For sourceRow = 12 To maxRowBudget

            ' Check if column AK (column 37) contains "True"
            If UCase(Trim(CStr(globalBudgetFYSheet.Cells(sourceRow, "AK").value))) = "TRUE" Then
                
                ' Get the lookup value from column A of the source row
                Dim lookupValue As String
                lookupValue = Trim(CStr(globalBudgetFYSheet.Cells(sourceRow, "A").value))
                
                If lookupValue <> "" Then
                    Dim targetRow As Long
                    Dim actionTaken As String
                    
                    ' Check if this lookup value already exists in FOR OPEX TAB
                    If existingLookupDict.Exists(lookupValue) Then
                        ' Update existing row
                        targetRow = existingLookupDict(lookupValue)
                        actionTaken = "Updated existing row"
                        updatedCount = updatedCount + 1
                    Else
                        ' Create new row
                        lastRowOpex = lastRowOpex + 1
                        targetRow = lastRowOpex
                        existingLookupDict(lookupValue) = targetRow ' Add to dictionary for future lookups
                        actionTaken = "Created new row"
                        copiedCount = copiedCount + 1
                    End If
                    
                    ' Copy/update this row from A to AJ
                    Dim sourceRange As Range
                    Dim targetRange As Range
                    Set sourceRange = globalBudgetFYSheet.Range("A" & sourceRow & ":AJ" & sourceRow)
                    Set targetRange = opexSheet.Range("A" & targetRow & ":AJ" & targetRow)

                    ' Copy as values only
                    targetRange.value = sourceRange.value
                    Debug.Print actionTaken & " " & targetRow & " in FOR OPEX TAB for lookup '" & lookupValue & "' from Budget-FY row " & sourceRow
                Else
                    Debug.Print "WARNING: Empty lookup value in Budget-FY row " & sourceRow & ", skipping"
                End If
            End If
        Next sourceRow
        Debug.Print "FOR OPEX TAB update completed: " & copiedCount & " new rows created, " & updatedCount & " existing rows updated"
    End If
    On Error GoTo 0

End Sub

Sub updateManualExportedSheet()

    ' Optimized version using dictionary-based bulk operations and proportional allocation

    ' Modified version of updateManual that works with the active sheet instead of globalMasterSheet
    ...
    ' Use active sheet instead of globalMasterSheet
    Dim activeSheet As Worksheet
    Set activeSheet = ThisWorkbook.activeSheet

    ' Unfilter both sheets first if there are any filters
    On Error Resume Next
    If globalBudgetFYSheet.AutoFilterMode Then
        globalBudgetFYSheet.ShowAllData
    End If
    ...
    globalBudgetFYSheet.Calculate
    If activeSheet.Range("A1").value <> "Individual P&L" Then
        MsgBox "Cannot update a summary."
        Exit Sub
    End If
    ...
    Dim arrMasterData As Variant
    arrMasterData = activeSheet.Range("A8:AM" & lastRowMaster).value
    ...
    For i = 1 To UBound(arrMasterData, 1)
        If arrMasterData(i, 1) <> "" And arrMasterData(i, 21) = "Manual Changes" Then ' Column U = 21
            ...
            ' DEBUG: Print details of each Manual Changes row found
            Debug.Print "=== MANUAL CHANGES ROW FOUND ==="
            Debug.Print "Row Number: " & (i + 7)
            Debug.Print "Column A (Ledger): " & Left(Trim(CStr(arrMasterData(i, 1))), 6)
            Debug.Print "Column B (Spend Category): " & Trim(CStr(arrMasterData(i, 2)))
            Debug.Print "Column C (Revenue Category): " & Trim(CStr(arrMasterData(i, 3)))
            Debug.Print "Column U (Source): " & Trim(CStr(arrMasterData(i, 21)))
            Debug.Print "Column AM (SummaryType): " & Trim(CStr(arrMasterData(i, 39)))
            Debug.Print "Monthly amounts G-R: " & arrMasterData(i, 7) & ", " & arrMasterData(i, 8) & ", " & arrMasterData(i, 9) & ", " & arrMasterData(i, 10) & ", " & arrMasterData(i, 11) & ", " & arrMasterData(i, 12) & ", " & arrMasterData(i, 13) & ", " & arrMasterData(i, 14) & ", " & arrMasterData(i, 15) & ", " & arrMasterData(i, 16) & ", " & arrMasterData(i, 17) & ", " & arrMasterData(i, 18)
            Debug.Print "================================"
        End If
    Next i
    Debug.Print "Found " & manualChangesDict.Count & " Manual Changes rows"
    ...
    ' --- Step 3: Get filter criteria from active sheet ---
    Dim company_code As String, cost_center As String, customer_segment As String, venue As String
    company_code = Trim(CStr(activeSheet.Cells(1, "G").value))
    cost_center = Trim(CStr(activeSheet.Cells(2, "G").value))
    customer_segment = Trim(CStr(activeSheet.Cells(3, "G").value))
    venue = Trim(CStr(activeSheet.Cells(4, "G").value))
    Debug.Print "Filter criteria - Company: '" & company_code & "', Cost Center: '" & cost_center & "', Customer Segment: '" & customer_segment & "', Venue: '" & venue & "'"
    ...
    For Each manualRowKey In manualChangesDict.keys
        ...
        ' DEBUG: Print transaction category being processed
        Debug.Print "=== PROCESSING MANUAL CHANGES ROW ==="
        Debug.Print "Row Number: " & manualRowKey
        Debug.Print "Ledger Account: '" & ledger_account & "'"
        Debug.Print "Spend Category: '" & spend_category & "'"
        Debug.Print "Revenue Category: '" & revenue_category & "'"
        Debug.Print "Transaction Category: '" & transactionCategory & "'"
        Debug.Print "Summary Type: '" & manualSummaryType & "'"
        ...
        ' Find matching rows in globalBudgetDict
        Call UpdateBudgetRowsWithAllocation(globalBudgetDict, company_code, cost_center, customer_segment, venue, transactionCategory, newAmounts, processedCount, maxRowBudget, ledger_account, spend_category, revenue_category, usGaapDict, manualSummaryType, activeSheet)
        Debug.Print "=== COMPLETED PROCESSING ROW " & manualRowKey & " ==="
    Next manualRowKey
    ...
    ' --- Step 7: Copy updated rows to FOR OPEX TAB sheet ---
    If processedCount > 0 Then
        Call CopyRowsToOpexTab(maxRowBudget)
    End If

    ' Get department from active sheet cell I1 and find appropriate column
    Dim department As String
    department = activeSheet.Range("I1").value

    ' Find department column for updatePnL call
    Dim deptCol As Long: deptCol = 25 ' Default fallback

    ' Try to find correct department column from Setup sheet
    On Error Resume Next
    If Not globalsetupSheet Is Nothing Then
        Dim setupLastRow As Long
        ...
        For setupRowIndex = 9 To setupLastRow
            ...
            If setupDept = department And setupCol <> "" And setupCol <> "0" Then
                deptCol = Range(setupCol & "1").Column
                Debug.Print "Found department column " & deptCol & " for department: " & department
                Exit For
            End If
        Next setupRowIndex
    End If
    On Error GoTo 0
    Call updatePnL(deptCol)

    ' Apply FilterSheets after updating
    Call FilterSheets
    Application.Calculation = xlCalculationAutomatic
    Application.ScreenUpdating = True
    Application.EnableEvents = True
    Dim elapsedTime As Double
    elapsedTime = Timer - startTime
    MsgBox "Manual update completed for exported sheet!" & vbCrLf & "Processed " & processedCount & " categories in " & Format(elapsedTime, "0.00") & " seconds", vbInformation

End Sub

' Apply FilterSheets macro to the specified sheet
Private Sub ApplyFilterSheetsToActiveSheet(targetSheet As Worksheet)
...
' Copy Budget-FY, Actual-FY, Setup, P&L-Parameters, and P&L sheets as last sheets
Private Sub CopySourceSheets(targetWorkbook As Workbook, departmentList As Object)

    ' Disable alerts to suppress naming conflict dialogs
    Application.DisplayAlerts = False
    Application.ScreenUpdating = False
    On Error Resume Next
    ...
    ' Copy all 6 sheets together to avoid external link issues
    Dim sheetsToClone As Variant
    sheetsToClone = Array("Budget-FY", "Actual-FY", "Setup", "P&L-Parameters", "P&L", "FOR OPEX TAB")
    sourceWorkbook.Sheets(sheetsToClone).Copy After:=targetWorkbook.Sheets(targetWorkbook.Sheets.Count)

    ' Filter Budget-FY and Actual-FY sheets after copying
    Call FilterBudgetAndActualSheets(targetWorkbook, departmentList)

    ' Fix button macro assignments in the copied sheets
    Call FixButtonMacroAssignments(targetWorkbook, sheetsToClone)

    ' Break all external links in the target workbook
    Call BreakAllExternalLinks(targetWorkbook)

    ' Hide P&L and P&L-Parameters sheets
    Call hideSpecificSheets(targetWorkbook)

    ' Re-enable alerts and screen updating
    Application.DisplayAlerts = True
    Application.ScreenUpdating = True
    On Error GoTo 0
    Debug.Print "Copied 6 source sheets together, filtered Budget-FY and Actual-FY, fixed button assignments, broke external links, and hid specified sheets"

End Sub

' Filter Budget-FY and Actual-FY sheets to only include rows matching the department list
Private Sub FilterBudgetAndActualSheets(targetWorkbook As Workbook, departmentList As Object)
    ' Filter Budget-FY and Actual-FY sheets using in-place dictionary filtering
    
    Dim sheetsToFilter As Variant
    sheetsToFilter = Array("Budget-FY", "Actual-FY")
    
    Dim sheetName As Variant
    For Each sheetName In sheetsToFilter
        On Error Resume Next
        Dim targetSheet As Worksheet
        Set targetSheet = targetWorkbook.Worksheets(CStr(sheetName))
        On Error GoTo 0
        
        If Not targetSheet Is Nothing Then
            Call FilterSheetByDepartment(targetSheet, departmentList, CStr(sheetName))
            Set targetSheet = Nothing
        End If
    Next sheetName
End Sub

' Filter a single sheet by department using dictionary approach
Private Sub FilterSheetByDepartment(targetSheet As Worksheet, departmentList As Object, sheetName As String)
    ' Get last row with data
    Dim lastRow As Long
    lastRow = targetSheet.Cells(targetSheet.Rows.Count, "A").End(xlUp).Row
    
    If lastRow < 12 Then
        Debug.Print "  " & sheetName & ": No data to filter (lastRow = " & lastRow & ")"
        Exit Sub
    End If
    
    ' Load all data from row 12 onwards into dictionary
    Dim allData As Variant
    allData = targetSheet.Range("A12:AN" & lastRow).value  ' A to AN (40 columns)
    
    ' Create dictionary to store filtered rows
    Dim filteredRows As Object
    Set filteredRows = CreateObject("Scripting.Dictionary")
    Dim filteredCount As Long: filteredCount = 0
    
    ' Filter data based on department list (column Y = index 25)
    Dim r As Long
    For r = 1 To UBound(allData, 1)
        ' Get department from column Y (index 25)
        Dim deptValue As String
        On Error Resume Next
        deptValue = Trim(CStr(allData(r, 25)))  ' Column Y = index 25
        On Error GoTo 0
        
        ' Include row ONLY if department explicitly matches our list (strict filtering)
        If departmentList.Exists(deptValue) Then
            ' Store the entire row
            Dim rowData As Variant
            ReDim rowData(1 To 40)  ' A:AN = 40 columns
            Dim c As Long
            For c = 1 To 40
                rowData(c) = allData(r, c)
            Next c
            filteredRows.Add filteredCount, rowData
    ...
    If lastRow >= 12 Then
        targetSheet.Range("A12:AN" & lastRow).ClearContents
    End If
    
    ' Write filtered data back to sheet
    If filteredCount > 0 Then
        Dim outputData As Variant
        ReDim outputData(1 To filteredCount, 1 To 40)
        
        Dim outputRow As Long
        For outputRow = 0 To filteredCount - 1
            Dim rowArray As Variant
            rowArray = filteredRows.Items()(outputRow)
            For c = 1 To 40
                outputData(outputRow + 1, c) = rowArray(c)
            Next c
        Next outputRow
        
        ' Write to sheet starting at row 12
        targetSheet.Range("A12").Resize(filteredCount, 40).value = outputData
    End If
    
    Debug.Print "  Filtered " & sheetName & ": " & (lastRow - 11) & " rows -> " & filteredCount & " rows (kept only department matches)"
End Sub


' Fix button macro assignments in copied sheets
Private Sub FixButtonMacroAssignments(targetWorkbook As Workbook, sheetNames As Variant)
    ...
    ' Loop through each copied sheet
    For i = LBound(sheetNames) To UBound(sheetNames)
        Set ws = targetWorkbook.Sheets(sheetNames(i))

        ' Loop through all shapes (including buttons) in the sheet
        For Each shp In ws.Shapes

            ' Check if shape has a macro assigned
            If shp.OnAction <> "" Then
                macroName = shp.OnAction

                ' Remove workbook reference if it exists (e.g., 'WorkbookName.xlsm'!MacroName -> MacroName)
                If InStr(macroName, "!") > 0 Then
                    newMacroName = Mid(macroName, InStr(macroName, "!") + 1)
                Else
                    newMacroName = macroName
                End If

                ' Remove single quotes if they exist
                newMacroName = Replace(newMacroName, "'", "")

                ' Assign the macro to the current workbook
                shp.OnAction = targetWorkbook.Name & "!" & newMacroName
                Debug.Print "Fixed button macro assignment: " & macroName & " -> " & shp.OnAction & " in sheet " & ws.Name
            End If
        Next shp
    Next i
    On Error GoTo 0
    Debug.Print "Completed fixing button macro assignments"

End Sub

' Break all external links in the target workbook
Private Sub BreakAllExternalLinks(targetWorkbook As Workbook)

    On Error Resume Next
    Dim linkSources As Variant
    linkSources = targetWorkbook.linkSources(xlExcelLinks)
    If Not IsEmpty(linkSources) Then
        Dim i As Long
        For i = 1 To UBound(linkSources)
            targetWorkbook.BreakLink Name:=linkSources(i), Type:=xlLinkTypeExcelLinks
        Next i
        Debug.Print "Broke " & UBound(linkSources) & " external links"
    Else
        Debug.Print "No external links found to break"
    End If
    On Error GoTo 0
    Debug.Print "Completed breaking external links"

End Sub

' Remove external links from copied sheets
Private Sub RemoveExternalLinksInSheets(targetWorkbook As Workbook, sheetNames As Variant)
        ...
        For Each cell In ws.usedRange
            If cell.HasFormula Then
                ...
                If InStr(formula, "'[") > 0 And InStr(formula, "].") > 0 Then
                    ...
                    ' Remove external workbook reference pattern: '[workbook.xlsm]Sheet' -> 'Sheet'
                    newFormula = formula

                    ' Use regex-like replacement to handle multiple external references in one formula
                    Dim startPos As Long, endPos As Long, bracketPos As Long
                    startPos = 1
                    Do While InStr(startPos, newFormula, "'[") > 0
                        startPos = InStr(startPos, newFormula, "'[")
                        bracketPos = InStr(startPos, newFormula, "]")
                        If bracketPos > 0 Then

                            ' Extract the part to replace: '[workbook.xlsm]' -> ''
                            Dim replaceThis As String
                            replaceThis = Mid(newFormula, startPos, bracketPos - startPos + 1)
                            newFormula = Replace(newFormula, replaceThis, "")
                            startPos = 1 ' Restart search from beginning
                        Else
                            Exit Do ' No closing bracket found
                        End If
                    Loop

                    ' Apply the cleaned formula
            ...
            End If
        Next cell
        Debug.Print "Removed external links from sheet: " & ws.Name
    Next i
    On Error GoTo 0
    Debug.Print "Completed removing external links. Fixed " & fixedCount & " formulas across " & UBound(sheetNames) + 1 & " sheets"

End Sub

' Hide P&L and P&L-Parameters sheets
Private Sub hideSpecificSheets(targetWorkbook As Workbook)

    On Error Resume Next

    ' Hide P&L sheet
    If Not targetWorkbook.Sheets("P&L") Is Nothing Then
        targetWorkbook.Sheets("P&L").Visible = xlSheetHidden
        Debug.Print "Hidden P&L sheet"
    End If

    ' Hide P&L-Parameters sheet
    If Not targetWorkbook.Sheets("P&L-Parameters") Is Nothing Then
        targetWorkbook.Sheets("P&L-Parameters").Visible = xlSheetHidden
        Debug.Print "Hidden P&L-Parameters sheet"
    End If
    On Error GoTo 0
    Debug.Print "Completed hiding specified sheets"

End Sub

' Copy all VBA modules to the new workbook
Private Sub CopyAllVBAModules(targetWorkbook As Workbook)

    ' Disable alerts to suppress any VBA-related prompts
    Application.DisplayAlerts = False
    Application.ScreenUpdating = False
    On Error Resume Next
    Dim sourceWorkbook As Workbook
    ...
    ' Loop through all VBA components in source workbook
    For Each sourceModule In sourceWorkbook.VBProject.VBComponents
        moduleName = sourceModule.Name

        ' Skip ThisWorkbook and worksheet modules
        If sourceModule.Type = 1 Then ' Standard module

            ' Export module to temporary file
            Dim tempPath As String
            tempPath = Environ("TEMP") & "\" & moduleName & ".bas"
            sourceModule.Export tempPath

            ' Import module to target workbook
            targetWorkbook.VBProject.VBComponents.Import tempPath

            ' Delete temporary file
            Kill tempPath
            Debug.Print "Copied VBA module: " & moduleName
        End If
    Next sourceModule
    ...
    Application.ScreenUpdating = True
    On Error GoTo 0
    Debug.Print "Completed copying VBA modules"
...
Sub ExportByDepartment()
    ...
    Call exportPnLs("By Department", "Z", "AA", "AB", "AD")

End Sub

Sub ExportByGroup()

    Call exportPnLs("By Group", "AF", "AG", "AH", "AI", "AJ")

End Sub

Sub getAllManualChanges()

    ' Function to process Manual Changes from all Individual P&L sheets

    ' 1. Find all sheets with "Individual P&L" in cell A1

    ' 2. Run updateManualExportedSheet() on each sheet
    Dim startTime As Double
    startTime = Timer
    ...
    Application.ScreenUpdating = False
    Application.EnableEvents = False
    Debug.Print "=== getAllManualChanges Started ==="
    
    ' Check password authentication before proceeding
    If Not UnlockAllSheets() Then
        Debug.Print "Password authentication failed - aborting getAllManualChanges"
        Application.Calculation = xlCalculationAutomatic
        Application.ScreenUpdating = True
        Application.EnableEvents = True
        Exit Sub
    ...
    ' Step 1: Create list of sheets with "Individual P&L" in cell A1
    Dim individualPnLSheets As Object
    Set individualPnLSheets = CreateObject("Scripting.Dictionary")
    Dim ws As Worksheet
    ...
    For Each ws In ThisWorkbook.Worksheets
        ...
        cellA1Value = Trim(CStr(ws.Cells(1, "A").value))
        On Error GoTo 0
        If cellA1Value = "Individual P&L" Then
            individualPnLSheets.Add ws.Name, ws
            sheetCount = sheetCount + 1
            Debug.Print "Found Individual P&L sheet: " & ws.Name
        End If
    Next ws
    Debug.Print "Total Individual P&L sheets found: " & sheetCount

    ' Check if any sheets were found
    If sheetCount = 0 Then
        MsgBox "No Individual P&L sheets found. No manual changes to process.", vbInformation
        Application.Calculation = xlCalculationAutomatic
        Application.ScreenUpdating = True
        Application.EnableEvents = True
        Exit Sub
    ...
    ' Step 2: Process each Individual P&L sheet
    Dim processedSheets As Long
    Dim skippedSheets As Long
    processedSheets = 0
    skippedSheets = 0
    Dim sheetKey As Variant
    For Each sheetKey In individualPnLSheets.keys
        Dim currentSheet As Worksheet
        Set currentSheet = individualPnLSheets(sheetKey)
        Debug.Print "=== Processing sheet: " & currentSheet.Name & " ==="

        ' Activate the sheet (required for updateManualExportedSheet)
        currentSheet.Activate

        ' Check if sheet has any Manual Changes before processing
        Dim hasManualChanges As Boolean
        hasManualChanges = CheckForManualChanges(currentSheet)
        If hasManualChanges Then
            Debug.Print "Found Manual Changes in sheet: " & currentSheet.Name

            ' Run updateManualExportedSheet on this sheet
            Call updateManualExportedSheet
            processedSheets = processedSheets + 1
            Debug.Print "Successfully processed sheet: " & currentSheet.Name
        Else
            Debug.Print "No Manual Changes found in sheet: " & currentSheet.Name & ", skipping"
            skippedSheets = skippedSheets + 1
        End If
        Debug.Print "=== Completed processing sheet: " & currentSheet.Name & " ==="
    Next sheetKey

    ' Step 3: Process Summary sheets
    Debug.Print "=== Step 3: Processing Summary Sheets ==="
    Dim summarySheetNames As Object
    Set summarySheetNames = CreateObject("Scripting.Dictionary")
    
    ' Collection of found Summary sheets for message box display
    Dim foundSummarySheets As Object
    Set foundSummarySheets = CreateObject("Scripting.Dictionary")

    ' Add the specific summary sheet names to look for
    summarySheetNames.Add "Summary", Nothing
    summarySheetNames.Add "Overall", Nothing
    summarySheetNames.Add "Summary2", Nothing
    summarySheetNames.Add "Summary3", Nothing
    summarySheetNames.Add "Summary4", Nothing
    summarySheetNames.Add "SummaryByCompany", Nothing
    Dim summaryProcessedCount As Long: summaryProcessedCount = 0
    Dim summarySkippedCount As Long: summarySkippedCount = 0

    ' Loop through all worksheets to find Summary sheets
    For Each ws In ThisWorkbook.Worksheets
        ' Get the value in cell A1
        On Error Resume Next
        Dim cellA1ValueSummary As String
        cellA1ValueSummary = Trim(CStr(ws.Cells(1, "A").value))
        On Error GoTo 0
        
        ' Check if cell A1 contains any of our summary keywords
        Dim isSummarySheet As Boolean: isSummarySheet = False
        Dim keyword As Variant
        For Each keyword In summarySheetNames.keys
            If InStr(1, cellA1ValueSummary, CStr(keyword), vbTextCompare) > 0 Then
                isSummarySheet = True
                Debug.Print "Found Summary sheet: " & ws.Name & " (A1 contains '" & keyword & "')"
                Exit For
            End If
        Next keyword

        ' Process if this is a summary sheet
        If isSummarySheet Then
            Debug.Print "=== Processing Summary sheet: " & ws.Name & " ==="

            ' Activate the summary sheet
            ws.Activate

            ' Get the column number from cell A3
            On Error Resume Next
            Dim columnNumber As Long
            columnNumber = CLng(ws.Range("A3").value)
            On Error GoTo 0
            If columnNumber > 0 Then
                Debug.Print "Running updatePnL(" & columnNumber & ") for sheet: " & ws.Name

                ' Run updatePnL with the column number from A3
                Call updatePnL(columnNumber)
                summaryProcessedCount = summaryProcessedCount + 1
                foundSummarySheets.Add ws.Name, "Processed (A3=" & columnNumber & ")"
                Debug.Print "Successfully processed Summary sheet: " & ws.Name
            Else
                Debug.Print "Invalid or missing column number in A3 for sheet: " & ws.Name & ", skipping"
                summarySkippedCount = summarySkippedCount + 1
                foundSummarySheets.Add ws.Name, "Skipped (Invalid A3=" & ws.Range("A3").value & ")"
            End If
            Debug.Print "=== Completed processing Summary sheet: " & ws.Name & " ==="
        End If
    Next ws
    Debug.Print "Summary sheets processed: " & summaryProcessedCount
    Debug.Print "Summary sheets skipped: " & summarySkippedCount
    ...
    Dim elapsedTime As Double
    elapsedTime = Timer - startTime
    Debug.Print "=== getAllManualChanges Completed ==="
    Debug.Print "Individual P&L sheets found: " & sheetCount
    Debug.Print "Individual P&L sheets processed: " & processedSheets
    Debug.Print "Individual P&L sheets skipped (no Manual Changes): " & skippedSheets
    Debug.Print "Summary sheets processed: " & summaryProcessedCount
    Debug.Print "Summary sheets skipped: " & summarySkippedCount
    Debug.Print "Total time elapsed: " & Format(elapsedTime, "0.00") & " seconds"
    
    ' Build Summary sheet details for message box
    Dim summaryDetails As String
    summaryDetails = ""
    If foundSummarySheets.Count > 0 Then
    ...
    MsgBox "Processing completed!" & vbCrLf & _
           "Individual P&L sheets found: " & sheetCount & vbCrLf & _
           "Individual P&L sheets processed: " & processedSheets & vbCrLf & _
           "Individual P&L sheets skipped: " & skippedSheets & vbCrLf & _
           "Summary sheets processed: " & summaryProcessedCount & vbCrLf & _
           "Summary sheets skipped: " & summarySkippedCount & vbCrLf & _
           "Time elapsed: " & Format(elapsedTime, "0.00") & " seconds" & _
           summaryDetails, vbInformation

End Sub


Private Function CheckForManualChanges(targetSheet As Worksheet) As Boolean

    ' Helper function to check if a sheet has any Manual Changes rows

    ' Returns True if Manual Changes found, False otherwise
    On Error Resume Next
    ...
    arrCheckData = targetSheet.Range("A8:U" & lastRowMaster).value

    ' Look for any row with "Manual Changes" in column U
    Dim i As Long
    For i = 1 To UBound(arrCheckData, 1)
        If arrCheckData(i, 1) <> "" And Trim(CStr(arrCheckData(i, 21))) = "Manual Changes" Then ' Column U = 21
            CheckForManualChanges = True
            Exit Function
        End If
    Next i
    CheckForManualChanges = False
    On Error GoTo 0
End Function
...
Private Sub HardCodeFormulasInSourceSheets()
    Debug.Print "=== HardCodeFormulasInSourceSheets Started ==="
    
    ' Process Budget-FY sheet
    If Not globalBudgetFYSheet Is Nothing Then
        Call HardCodeFormulasInSheet(globalBudgetFYSheet, "Budget-FY")
    Else
        Debug.Print "WARNING: globalBudgetFYSheet is Nothing"
    End If
    
    ' Process Actual-FY sheet
    If Not globalActualFYSheet Is Nothing Then
        Call HardCodeFormulasInSheet(globalActualFYSheet, "Actual-FY")
    Else
        Debug.Print "WARNING: globalActualFYSheet is Nothing"
    End If
    
    Debug.Print "=== HardCodeFormulasInSourceSheets Completed ==="
End Sub

' Hard code formulas to values in a specific sheet
Private Sub HardCodeFormulasInSheet(targetSheet As Worksheet, sheetName As String)
    ...
    If lastRow < 12 Then
        Debug.Print "  No data to process in " & sheetName & " (lastRow = " & lastRow & ")"
        Exit Sub
    End If
    
    Debug.Print "  Converting formulas to values from row 12 to " & lastRow & " in " & sheetName
    
    ' Define column ranges to process (A to AJ except V, Z, AA)
    ' A-U (columns 1-21), skip V (22), W-Y (23-25), skip Z (26), skip AA (27), AB-AJ (28-36)
    Dim columnsToProcess As Variant
    columnsToProcess = Array( _
        Array(1, 21), _
        Array(23, 25), _
        Array(28, 36) _
    )
    ...
    For rangeIndex = LBound(columnsToProcess) To UBound(columnsToProcess)
        Dim startCol As Long, endCol As Long
        startCol = columnsToProcess(rangeIndex)(0)
        endCol = columnsToProcess(rangeIndex)(1)
        
        ' Convert column numbers to letters for range reference
        Dim startColLetter As String, endColLetter As String
        startColLetter = Split(targetSheet.Cells(1, startCol).Address, "$")(1)
        endColLetter = Split(targetSheet.Cells(1, endCol).Address, "$")(1)
        
        ' Define the range to process
        Dim rangeToProcess As Range
        Set rangeToProcess = targetSheet.Range(startColLetter & "12:" & endColLetter & lastRow)
        
        Debug.Print "    Processing range: " & startColLetter & "12:" & endColLetter & lastRow
        
        ' Convert formulas to values
        On Error Resume Next
        rangeToProcess.value = rangeToProcess.value
        If Err.Number <> 0 Then
            Debug.Print "    ERROR converting range " & startColLetter & "12:" & endColLetter & lastRow & " - " & Err.Description
            Err.Clear
        Else
            Debug.Print "    Successfully converted range " & startColLetter & "12:" & endColLetter & lastRow
        End If
        On Error GoTo 0
    Next rangeIndex
    
    Debug.Print "  Completed processing " & sheetName
End Sub


Private Sub LockSummarySheets(targetWorkbook As Workbook)

    '-----------------------------

    ' LockSummarySheets: Lock cells and protect sheets that match summary keywords
    ...
    Dim summaryKeyword As Variant
    For Each ws In targetWorkbook.Worksheets

        ' Skip P&L sheet
        If ws.Name = "P&L" Then
            Debug.Print "Skipping P&L sheet from summary locking"
            GoTo NextSummarySheet
        End If
        ...
        ' Check if A1 contains any summary keywords
        foundKeyword = ""
        For Each summaryKeyword In summarySheetNames.keys
            If InStr(1, cellA1Value, CStr(summaryKeyword), vbTextCompare) > 0 Then
                foundKeyword = CStr(summaryKeyword)
                Exit For
            End If
        Next summaryKeyword

        ' If summary keyword found, lock and protect the sheet
        If foundKeyword <> "" Then
            foundSummarySheets.Add ws.Name, cellA1Value
            Debug.Print "Found summary sheet: " & ws.Name & " (A1: '" & cellA1Value & "', Keyword: '" & foundKeyword & "')"
            ...
            Debug.Print "  Locked all cells in sheet: " & ws.Name
            ...
            If Err.Number = 0 Then
                Debug.Print "  Successfully protected sheet: " & ws.Name
            Else
                Debug.Print "  ERROR protecting sheet " & ws.Name & ": " & Err.Description
                Err.Clear
            End If
            On Error GoTo 0
        End If
NextSummarySheet:
    Next ws
    Debug.Print "=== Summary Sheet Processing Complete ==="
    Debug.Print "Total summary sheets found and locked: " & foundSummarySheets.Count

    ' Debug output of found sheets
    If foundSummarySheets.Count > 0 Then
        Debug.Print "Summary sheets processed:"
        Dim sheetKey As Variant
        For Each sheetKey In foundSummarySheets.keys
            Debug.Print "  - " & sheetKey & " (A1: '" & foundSummarySheets(sheetKey) & "')"
        Next sheetKey
    Else
        Debug.Print "No summary sheets found to lock."
    End If

End Sub


Public Function UnlockAllSheets() As Boolean

    '-----------------------------
    ' UnlockAllSheets: Unlock and unprotect sheets that match specific keywords in A1
    ' Returns True if successful, False if password authentication fails
    '-----------------------------
    Debug.Print "=== Unlocking All Protected Sheets ==="
    
    ' Prompt for password
    Dim inputPassword As String
    inputPassword = InputBox("Enter password to unlock sheets:", "Sheet Protection Password")
    
    ' Handle if user cancels, enters nothing, or enters wrong password
    If inputPassword = "" Then
        Debug.Print "Password input cancelled or empty - aborting unlock operation"
        MsgBox "Unlock operation cancelled. Password is required to proceed.", vbExclamation, "Authentication Required"
    ...
    Debug.Print "Password authenticated successfully - proceeding with unlock"

    ' Collection of keywords to look for in cell A1
    Dim unlockKeywords As Object
    Set unlockKeywords = CreateObject("Scripting.Dictionary")

    ' Add the keywords to look for
    unlockKeywords.Add "Summary", Nothing
    unlockKeywords.Add "Overall", Nothing
    unlockKeywords.Add "Summary2", Nothing
    unlockKeywords.Add "Summary3", Nothing
    unlockKeywords.Add "Summary4", Nothing
    unlockKeywords.Add "SummaryByCompany", Nothing
    unlockKeywords.Add "Individual P&L", Nothing

    ' Collection of found sheets for reporting
    Dim foundSheets As Object
    Set foundSheets = CreateObject("Scripting.Dictionary")
    ...
    For Each ws In ThisWorkbook.Worksheets
        ...
        ' Check if A1 contains any unlock keywords
        foundKeyword = ""
        For Each keyword In unlockKeywords.keys
            If InStr(1, cellA1Value, CStr(keyword), vbTextCompare) > 0 Then
                foundKeyword = CStr(keyword)
                Exit For
            End If
        Next keyword

        ' If keyword found, unlock and unprotect the sheet
        If foundKeyword <> "" Then
            foundSheets.Add ws.Name, cellA1Value
            Debug.Print "Found sheet to unlock: " & ws.Name & " (A1: '" & cellA1Value & "', Keyword: '" & foundKeyword & "')"

            ' Unprotect sheet with the authenticated password
            On Error Resume Next
            ws.Unprotect inputPassword
            If Err.Number = 0 Then
                Debug.Print "  Successfully unprotected sheet: " & ws.Name
            Else
                Debug.Print "  Sheet was not protected or error unprotecting: " & ws.Name & " - " & Err.Description
                Err.Clear
            End If
            On Error GoTo 0
        End If
    Next ws
    Debug.Print "=== Unlock Process Complete ==="
    Debug.Print "Total sheets found and unlocked: " & foundSheets.Count

    ' Show message box with results
    Dim resultMessage As String
    If foundSheets.Count > 0 Then
        resultMessage = "Successfully unlocked " & foundSheets.Count & " sheet(s):" & vbCrLf & vbCrLf
        Dim sheetKey As Variant
        For Each sheetKey In foundSheets.keys
            resultMessage = resultMessage & "- " & sheetKey & " (A1: '" & foundSheets(sheetKey) & "')" & vbCrLf
        Next sheetKey
    Else
        resultMessage = "No sheets found matching the unlock criteria." & vbCrLf & vbCrLf & _
                       "Sheets must have one of these keywords in cell A1:" & vbCrLf & _
                       "Summary, Overall, Summary2, Summary3, Summary4, SummaryByCompany, Individual P&L"
    End If

    'MsgBox resultMessage, vbInformation, "Unlock All Sheets - Complete"

    ' Debug output of found sheets
    If foundSheets.Count > 0 Then
        Debug.Print "Sheets unlocked:"
        For Each sheetKey In foundSheets.keys
            Debug.Print "  - " & sheetKey & " (A1: '" & foundSheets(sheetKey) & "')"
        Next sheetKey
    Else
        Debug.Print "No sheets found to unlock."
    End If
    
    UnlockAllSheets = True

End Function


Public Sub UnhideButtons(Optional targetWorkbook As Workbook = Nothing)

    '-----------------------------

    ' UnhideButtons: Unhide UnlockButton and GetAdjButton form controls

    '-----------------------------
    Debug.Print "=== Unhiding Form Control Buttons ==="
    Dim ws As Worksheet
    Dim shp As Shape
    Dim buttonsFound As Long: buttonsFound = 0
    Dim buttonsUnhidden As Long: buttonsUnhidden = 0
    Dim workbookToProcess As Workbook

    ' Determine which workbook to process
    If targetWorkbook Is Nothing Then
        Set workbookToProcess = ThisWorkbook
        Debug.Print "Processing active workbook: " & workbookToProcess.Name
    Else
        Set workbookToProcess = targetWorkbook
        Debug.Print "Processing specified workbook: " & workbookToProcess.Name
    End If

    ' Search through all worksheets in the workbook
    For Each ws In workbookToProcess.Worksheets
        Debug.Print "Searching for buttons in sheet: " & ws.Name

        ' Search through all shapes in the worksheet
        For Each shp In ws.Shapes

            ' Check if the shape is one of our target buttons
            If shp.Name = "UnlockButton" Or shp.Name = "GetAdjButton" Or shp.Name = "RefreshButton" Or shp.Name = "LockButton" Then
                buttonsFound = buttonsFound + 1
                Debug.Print "  Found button: " & shp.Name & " in sheet: " & ws.Name

                ' Check current visibility status
                Debug.Print "    Current visibility: " & shp.Visible

                ' Unhide the button
                On Error Resume Next
                shp.Visible = msoTrue
                If Err.Number = 0 Then
                    buttonsUnhidden = buttonsUnhidden + 1
                    Debug.Print "    Successfully unhidden button: " & shp.Name
                Else
                    Debug.Print "    ERROR unhiding button " & shp.Name & ": " & Err.Description
                    Err.Clear
                End If
                On Error GoTo 0
            End If
        Next shp
    Next ws
    Debug.Print "=== Button Unhiding Complete ==="
    Debug.Print "Total buttons found: " & buttonsFound
    Debug.Print "Total buttons unhidden: " & buttonsUnhidden

    ' Show result message
    Dim resultMessage As String
    If buttonsFound > 0 Then
        resultMessage = "Button unhiding completed!" & vbCrLf & vbCrLf & _
                       "Buttons found: " & buttonsFound & vbCrLf & _
                       "Buttons unhidden: " & buttonsUnhidden
        If buttonsUnhidden < buttonsFound Then
            resultMessage = resultMessage & vbCrLf & vbCrLf & _
                           "Some buttons may have already been visible or encountered errors."
        End If
    Else
        resultMessage = "No UnlockButton, GetAdjButton, RefreshButton, or LockButton form controls found in any worksheet." & vbCrLf & vbCrLf & _
                       "Please verify that these buttons exist and have the correct names."
    End If
    'MsgBox resultMessage, vbInformation, "Unhide Buttons - Complete"

End Sub


Private Sub LockIndividualPLSheets(targetWorkbook As Workbook)

    '-----------------------------

    ' LockIndividualPLSheets: Protect sheets that have "Individual P&L" in cell A1

    ' Note: Does not change cell lock properties - assumes they are already set manually

    '-----------------------------
    Debug.Print "=== Processing Individual P&L Sheets ==="

    ' Collection of found Individual P&L sheets for reporting
    Dim foundIndividualPLSheets As Object
    Set foundIndividualPLSheets = CreateObject("Scripting.Dictionary")
    ...
    For Each ws In targetWorkbook.Worksheets
        ...
        ' Skip P&L sheet
        If ws.Name = "P&L" Then
            Debug.Print "Skipping P&L sheet from Individual P&L locking"
            GoTo NextIndividualSheet
        End If
        ...
        ' Check if A1 contains "Individual P&L"
        If InStr(1, cellA1Value, "Individual P&L", vbTextCompare) > 0 Then
            foundIndividualPLSheets.Add ws.Name, cellA1Value
            Debug.Print "Found Individual P&L sheet: " & ws.Name & " (A1: '" & cellA1Value & "')"
            ...
            ws.Protect "Budget", DrawingObjects:=True, Contents:=True, Scenarios:=True, _
                      ...
                      AllowSorting:=False, AllowFiltering:=False, AllowUsingPivotTables:=False
            If Err.Number = 0 Then
                Debug.Print "  Successfully protected Individual P&L sheet: " & ws.Name
            Else
                Debug.Print "  ERROR protecting Individual P&L sheet " & ws.Name & ": " & Err.Description
                Err.Clear
            End If
            On Error GoTo 0
        End If
NextIndividualSheet:
    Next ws
    Debug.Print "=== Individual P&L Sheet Processing Complete ==="
    Debug.Print "Total Individual P&L sheets found and locked: " & foundIndividualPLSheets.Count

    ' Debug output of found sheets
    If foundIndividualPLSheets.Count > 0 Then
        Debug.Print "Individual P&L sheets processed:"
        Dim sheetKey As Variant
        For Each sheetKey In foundIndividualPLSheets.keys
            Debug.Print "  - " & sheetKey & " (A1: '" & foundIndividualPLSheets(sheetKey) & "')"
        Next sheetKey
    Else
        Debug.Print "No Individual P&L sheets found to lock."
    End If
...
Sub LockAllSheets()
    ...
    ' Lock summary sheets
    Debug.Print "Locking summary sheets in workbook: " & ThisWorkbook.Name
    Call LockSummarySheets(Activebook)

    ' Lock Individual P&L sheets
    Debug.Print "Locking Individual P&L sheets in workbook: " & ThisWorkbook.Name
    Call LockIndividualPLSheets(Activebook)
...
Sub refreshPnLs()
    ...
    ' refreshPnLs: Refresh P&L data for all qualifying sheets

    ' Processes sheets with specific A1 values and runs updatePnL with colNum from A3

    '-----------------------------
    Dim ws As Worksheet
    Dim cellA1Value As String
    Dim cellA3Value As Variant
    Dim sheetColNum As Long
    Dim processedCount As Long
    Dim startTime As Double
    Dim elapsedTime As Double
    ...
    startTime = Timer
    processedCount = 0
    Debug.Print "=== refreshPnLs Started ==="
   
    ' Check password authentication before proceeding
    If Not UnlockAllSheets() Then
        Debug.Print "Password authentication failed - aborting refreshPnLs"
        Application.Calculation = xlCalculationAutomatic
        Application.ScreenUpdating = True
        Application.EnableEvents = True
        Exit Sub
    ...
    excludeSheets.Add "Budget-FY", True
    excludeSheets.Add "Actual-FY", True
    excludeSheets.Add "Setup", True
    excludeSheets.Add "P&L-Parameters", True
    excludeSheets.Add "FOR OPEX TAB", True
    excludeSheets.Add "P&L", True

    ' Define valid A1 values
    Dim validA1Values As Object
    Set validA1Values = CreateObject("Scripting.Dictionary")
    validA1Values.Add "Summary", True
    validA1Values.Add "Overall", True
    validA1Values.Add "Summary2", True
    validA1Values.Add "Summary3", True
    validA1Values.Add "Summary4", True
    validA1Values.Add "SummaryByCompany", True
    validA1Values.Add "Individual P&L", True

    ' Process each worksheet
    For Each ws In ThisWorkbook.Worksheets

        ' Skip excluded sheets
        If excludeSheets.Exists(ws.Name) Then
            Debug.Print "Skipping excluded sheet: " & ws.Name
            GoTo NextSheet
        End If
        ...
        cellA1Value = Trim(CStr(ws.Cells(1, "A").value))
        On Error GoTo 0
        Debug.Print "  A1 value: '" & cellA1Value & "'"

        ' Check if A1 value is in our valid list
        If validA1Values.Exists(cellA1Value) Then

            ' Get A3 value and convert to column number
            On Error Resume Next
            cellA3Value = ws.Cells(3, "A").value
            On Error GoTo 0
            If IsNumeric(cellA3Value) Then
                sheetColNum = CLng(cellA3Value)
            Else
                sheetColNum = 25 ' Use default column 25 as fallback
                Debug.Print "  WARNING: A3 is not numeric ('" & cellA3Value & "'), using default colNum: 25"
            End If
            Debug.Print "  A3 value: " & cellA3Value & " -> Using colNum: " & sheetColNum
            Debug.Print "  Running updatePnL(" & sheetColNum & ") for sheet: " & ws.Name

            ' Run updatePnL with the determined column number
            Call updatePnL(sheetColNum)
            processedCount = processedCount + 1
            Debug.Print "  Completed updatePnL for sheet: " & ws.Name
        Else
            Debug.Print "  Skipping - A1 value '" & cellA1Value & "' not in valid list"
        End If
NextSheet:
    Next ws
    ...
    Application.EnableEvents = True
    elapsedTime = Timer - startTime
    Debug.Print "=== refreshPnLs Completed ==="
    Debug.Print "Total sheets processed: " & processedCount
    Debug.Print "Time elapsed: " & Format(elapsedTime, "0.00") & " seconds"
    MsgBox "P&L refresh completed!" & vbCrLf & _
...
Public Sub FilterSheetsNewWorkbook()
...
Sub UnfilterActiveSheetNewWorkbook()
...
Private Sub PutMacroInButton(newWorkbook As Workbook)

    '-----------------------------
    ' PutMacroInButton: Assign macros to FilterButton and UnfilterButton in qualifying sheets
    ' Processes sheets with specific A1 values and assigns macros to buttons
    '-----------------------------
    
    Debug.Print "=== Assigning Macros to Buttons ==="
    
    ' Define valid A1 values for sheets that should have macro assignments
    Dim validA1Values As Object
    Set validA1Values = CreateObject("Scripting.Dictionary")
    validA1Values.Add "Summary", True
    validA1Values.Add "Overall", True
    ...
    Dim ws As Worksheet
    Dim cellA1Value As String
    Dim processedSheets As Long: processedSheets = 0
    Dim buttonsProcessed As Long: buttonsProcessed = 0
    
    ' Process each worksheet in the target workbook
    For Each ws In newWorkbook.Worksheets
        ...
        Debug.Print "Checking sheet: " & ws.Name & " (A1: '" & cellA1Value & "')"
        ...
        If validA1Values.Exists(cellA1Value) Then
            ...
            ' Activate the sheet to work with its buttons
            ws.Activate
            
            ' Process all shapes in the worksheet
            Dim shp As Shape
            For Each shp In ws.Shapes
                
                ' Assign macro to FilterButton
                If shp.Name = "FilterButton" Then
                    On Error Resume Next
                    shp.OnAction = "FilterSheetsNewWorkbook"
                    If Err.Number = 0 Then
                        Debug.Print "    Assigned FilterSheets macro to FilterButton in sheet: " & ws.Name
                        buttonsProcessed = buttonsProcessed + 1
                    Else
                        Debug.Print "    ERROR assigning FilterSheets macro to FilterButton in sheet: " & ws.Name & " - " & Err.Description
                        Err.Clear
                    End If
                    On Error GoTo 0
                End If
                
                ' Assign macro to UnfilterButton
                If shp.Name = "UnfilterButton" Then
                    On Error Resume Next
                    shp.OnAction = "UnfilterActiveSheetNewWorkbook"
                    If Err.Number = 0 Then
                        Debug.Print "    Assigned UnfilterActiveSheet macro to UnfilterButton in sheet: " & ws.Name
                        buttonsProcessed = buttonsProcessed + 1
                    Else
                        Debug.Print "    ERROR assigning UnfilterActiveSheet macro to UnfilterButton in sheet: " & ws.Name & " - " & Err.Description
                        Err.Clear
                    End If
                    On Error GoTo 0
                End If
        ...
        Else
            Debug.Print "  Skipping sheet: " & ws.Name & " (A1 value '" & cellA1Value & "' not in valid list)"
        End If
        
    Next ws
    
    Debug.Print "=== Macro Assignment Complete ==="
    Debug.Print "Total qualifying sheets processed: " & processedSheets
    Debug.Print "Total buttons with macros assigned: " & buttonsProcessed
    
    Call LockAllSheets

End Sub

Sub clearTabColor(newWorkbook As Workbook)
    ' Clear tab colors for sheets with "Individual P&L" in cell A1
    Debug.Print "=== Clearing Tab Colors ==="
    
    Dim ws As Worksheet
    Dim sheetsProcessed As Long: sheetsProcessed = 0
    
    For Each ws In newWorkbook.Worksheets
        ' Check if cell A1 contains "Individual P&L"
        On Error Resume Next
        Dim cellA1Value As String
        cellA1Value = Trim(CStr(ws.Cells(1, "A").value))
        On Error GoTo 0
        ...
        If cellA1Value = "Individual P&L" Then
            ...
            If Err.Number = 0 Then
                Debug.Print "Cleared tab color for sheet: " & ws.Name
                sheetsProcessed = sheetsProcessed + 1
            Else
                Debug.Print "Error clearing tab color for sheet: " & ws.Name & " - " & Err.Description
                Err.Clear
            End If
            On Error GoTo 0
        End If
    ...
    Debug.Print "Total sheets with tab colors cleared: " & sheetsProcessed
End Sub


Sub hideSpecificSheetsOther(Optional targetWorkbook As Workbook = Nothing)
    ' Hide Budget-FY, Actual-FY, and FOR OPEX TAB sheets
    Debug.Print "=== Hiding Specific Sheets ==="
    ...
    sheetsToHide = Array("Budget-FY", "Actual-FY", "FOR OPEX TAB")
    
    Dim i As Long
    Dim ws As Worksheet
    Dim sheetsProcessed As Long: sheetsProcessed = 0
    
    For i = 0 To UBound(sheetsToHide)
        On Error Resume Next
        Set ws = wb.Worksheets(sheetsToHide(i))
        If Not ws Is Nothing Then
            ' Only hide if sheet is currently visible
            If ws.Visible = xlSheetVisible Then
                ws.Visible = xlSheetHidden
                Debug.Print "Hidden sheet: " & sheetsToHide(i)
                sheetsProcessed = sheetsProcessed + 1
            Else
                Debug.Print "Sheet already hidden: " & sheetsToHide(i)
            End If
        Else
            Debug.Print "Sheet not found: " & sheetsToHide(i)
        End If
        Set ws = Nothing
        Err.Clear
        On Error GoTo 0
    ...
    Debug.Print "Total sheets hidden: " & sheetsProcessed
End Sub

Sub unhideSpecificSheets(Optional targetWorkbook As Workbook = Nothing)
    ' Unhide Budget-FY, Actual-FY, and FOR OPEX TAB sheets
    Debug.Print "=== Unhiding Specific Sheets ==="
    ...
    sheetsToUnhide = Array("Budget-FY", "Actual-FY", "FOR OPEX TAB")
    
    Dim i As Long
    Dim ws As Worksheet
    Dim sheetsProcessed As Long: sheetsProcessed = 0
    
    For i = 0 To UBound(sheetsToUnhide)
        On Error Resume Next
        Set ws = wb.Worksheets(sheetsToUnhide(i))
        If Not ws Is Nothing Then
            ' Only unhide if sheet is currently hidden
            If ws.Visible <> xlSheetVisible Then
                ws.Visible = xlSheetVisible
                Debug.Print "Unhidden sheet: " & sheetsToUnhide(i)
                sheetsProcessed = sheetsProcessed + 1
            Else
                Debug.Print "Sheet already visible: " & sheetsToUnhide(i)
            End If
        Else
            Debug.Print "Sheet not found: " & sheetsToUnhide(i)
        End If
        Set ws = Nothing
        Err.Clear
        On Error GoTo 0
    ...
    Debug.Print "Total sheets unhidden: " & sheetsProcessed
End Sub

Private Sub collapseSetupSheet(targetWorkbook As Workbook)
    ' Collapse rows and columns in the Setup sheet of the target workbook
    Debug.Print "Collapsing Setup sheet in workbook: " & targetWorkbook.Name
    
    ' Find the Setup sheet in the target workbook
    Dim setupSheet As Worksheet
    Set setupSheet = Nothing
    
    ' Look for Setup sheet (try common names)
    Dim ws As Worksheet
    For Each ws In targetWorkbook.Worksheets
        If UCase(ws.Name) = "SETUP" Or UCase(ws.Name) = "SETUP SHEET" Or InStr(UCase(ws.Name), "SETUP") > 0 Then
            Set setupSheet = ws
            Exit For
        End If
    Next ws
    ...
    If setupSheet Is Nothing Then
        Debug.Print "WARNING: Setup sheet not found in workbook: " & targetWorkbook.Name
        Exit Sub
    End If
    
    ' Activate the setup sheet and collapse outlines
    On Error Resume Next
    setupSheet.Activate
    
    ' Collapse all outlined rows and columns to level 1 (most collapsed)
    setupSheet.Outline.ShowLevels RowLevels:=1, ColumnLevels:=1
    
    ' Select a safe cell (A1) after collapsing
    setupSheet.Range("A1").Select
    
    Debug.Print "Successfully collapsed Setup sheet: " & setupSheet.Name
    On Error GoTo 0
