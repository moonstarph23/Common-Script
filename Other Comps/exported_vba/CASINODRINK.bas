...
Sub CopyCDSheet()
    ...
    Set targetSheet = mainWorkbook.Sheets("Casino Drink") ' Change Cover to your target sheet name
    ...
    ' Apply the filter'
    Rng.AutoFilter Field:=4, Criteria1:="*COMP Casino Drink*"
    ...
    filesSheet.Range("C7").Value = "Successful"
    Exit Sub
ErrorHandler:

    ' If there is an error, write the error message in Files sheet cell B3
    filesSheet.Range("C7").Value = "Error: " & Err.Description
    Application.DisplayAlerts = originalDisplayAlerts
