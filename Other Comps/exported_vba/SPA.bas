Sub CopySPSheet()
    ...
    Set targetSheet = mainWorkbook.Sheets("COMP SPA PACKAGE") ' Change Cover to your target sheet name
    ...
    ' Apply the filter'
    Rng.AutoFilter Field:=4, Criteria1:="*Spa FD Pkg*"
    ...
    filesSheet.Range("C10").Value = "Successful"
    Exit Sub
ErrorHandler:

    ' If there is an error, write the error message in Files sheet cell B3
    filesSheet.Range("C10").Value = "Error: " & Err.Description
    Application.DisplayAlerts = originalDisplayAlerts
